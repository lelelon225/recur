...
**Backend** package convention is domain-first: `ch.noseryoung.domain.recur.<domain>.{controller,service,repository,model,dto,enums,exceptions}`, one package per domain (`task`, `group`, `notification`, `auth` — `auth` additionally has `security/jwt` and `security/oauth2`). Cross-cutting code that isn't clearly owned by one domain lives under `ch.noseryoung.domain.recur.shared`: `shared/exceptions/` (`GlobalExceptionHandler`, `ErrorResponse`), `shared/security/` (`SecurityConfig`), `shared/service/` (`EmailService`, used by both `auth` and `notification`). `Task` uses a `Task.OnCreate` validation group so `@NotBlank`/`@NotNull`/`@Future` only apply on POST, not PATCH (PATCH is a partial update — null means "don't change"). All Task queries are scoped by owner from `SecurityContextHolder`; deletion is only allowed for archived tasks.
