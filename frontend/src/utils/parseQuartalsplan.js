...
function matchHeaderKey(headerText) {
    ...
    if (normalized === "kw")
        return "kw";
    if (normalized.includes("kompetenzabnahme"))
        return "kompetenzabnahme";
    if (normalized.includes("unterrichtsblock"))
        return "unterrichtsblock";
    if (normalized.includes("modul"))
        return "modul";
    if (normalized.includes("prufung"))
        return "pruefungen";
    if (normalized.includes("abgabe"))
        return "abgabetermine";
    if (normalized.includes("lektion"))
        return "lektion";
    return null;
}
...
function extractKwFromText(text) {
...
function buildRowsFromCells(columnOrder, dataRows) {
...
const LEGACY_SMARTLEARN_COLUMN_ORDER = [
    "kw",
    "unterrichtsblock",
    "modul",
    "pruefungen",
...
function parseHtmlTable(html) {
    ...
    const columnOrder = hasRecognizedHeader
        ? headerKeys
        : LEGACY_SMARTLEARN_COLUMN_ORDER;
    const dataTrs = hasRecognizedHeader ? trs.slice(1) : trs;
    const dataRows = dataTrs.map((tr) => Array.from(tr.querySelectorAll("td, th")).map(cellText));
...
export function parseClipboardTable(html, text) {
    if (html) {
...
export function detectReferenceYear(rows) {
...
export function fridayOfIsoWeek(week, year) {
...
function draftFromDate(date, dateSource) {
...
export function buildDraftTasks(rows, referenceYear) {
    const drafts = [];
    for (const row of rows) {
        ...
        if (row.pruefungen) {
            ...
            drafts.push({
                ...
                description: truncate([row.modul, `KW ${row.kw}`].filter(Boolean).join(" · "), DESCRIPTION_MAX),
                dateUntil,
                dateSource,
                selected,
                warning,
...
export function recomputeComputedDates(drafts, referenceYear) {
    return drafts.map((draft) => {
        if (draft.dateSource !== "computed")
            return draft;
        const { dateUntil, selected, warning } = draftFromDate(fridayOfIsoWeek(draft.kw, referenceYear), "computed");
