...
const validationSchema = yup.object().shape({
    ...
    startDate: yup
        ...
        .transform((value, originalValue) => (originalValue === "" ? null : value))
        .nullable()
        .test("start-pair", "Datum und Uhrzeit müssen beide gesetzt sein oder beide leer bleiben", function (value) {
    ...
    startTimeOfDay: yup
        ...
        .test("time-format", "Ungültiges Zeitformat (Format muss HH:mm sein)", (value) => !value || /^([0-1][0-9]|2[0-3]):[0-5][0-9]$/.test(value)),
});
...
function EditTaskForm({ task, onClose, onTaskUpdated }) {
    ...
    return (<Formik initialValues={{
      ...
      {({ values, handleChange, handleBlur, handleSubmit: formikHandleSubmit, errors, touched, dirty, isValid, }) => (<AppDialog open onClose={onClose} title="Bearbeite die Details der Aufgabe" onSubmit={() => formikHandleSubmit()} loading={loading} submitDisabled={loading || !dirty || !isValid}>
          <Form onSubmit={formikHandleSubmit} values={values} handleChange={handleChange} handleBlur={handleBlur} errors={errors} touched={touched} className="addTaskForm"/>
        </AppDialog>)}
    </Formik>);
}
