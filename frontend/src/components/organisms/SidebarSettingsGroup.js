...
function SidebarSettingsGroup({ SettingsItem }) {
    return (<SidebarGroup>
                ...
                <Separator className="my-2"/>
                    <SidebarMenu>
                        <SidebarMenuItem>
                            ...
                            {SettingsItem.map((item, index) => (<SidebarSettingsItem key={index} label={item.label} Icon={item.Icon} onClick={item.onClick}/>))}
                    </SidebarMenu>
        </SidebarGroup>);
}
