...
function HomePage() {
    const { tasks, loading, handleToggleFavorite, handleToggleArchive, handleResetProgress, handleDelete, handleToggleDone, handleUpdateTask, } = useTasksContext();
    ...
    if (tasks.length === 0) {
        return (<Empty icon={() => <FilePlus2 className="w-12 h-12 text-gray-400"/>} title="Keine Habits" description="Es gibt derzeit keine Habits." buttonText="Habit erstellen" onButtonClick={openAddTaskForm}/>);
    }
    return (<div>
      ...
      <TaskCardGrid sortedTasks={sortedTasks} handlers={handlers} direction="row"/>
    </div>);
}
