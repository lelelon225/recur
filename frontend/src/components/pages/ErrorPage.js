...
function ErrorPage({ resetErrorBoundary, errorCode, errorMessage, buttonText, fullScreen = true }) {
    const { icon: Icon, message } = ERROR_OPTIONS[errorCode ?? 500] ?? DEFAULT_ERROR_OPTION;
    return (<div className={cn("flex w-full flex-col items-center justify-center gap-6 bg-background px-4 py-8", fullScreen ? "min-h-svh" : "min-h-[50vh]")}>
            <Card className="w-full max-w-sm border-none shadow-lg">
                ...
                {resetErrorBoundary && (<CardFooter className="flex flex-col justify-center pt-4">
