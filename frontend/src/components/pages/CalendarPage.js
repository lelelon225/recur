...
import { getWeekDays, getMonthGrid, getWeekLabel, getMonthLabel, categoryDot, } from "@/utils/calendarGrid";
...
function CalendarGrid() {
    ...
    return (<div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-0.5 rounded-full border border-border p-0.5">
            <Button variant="ghost" size="icon-sm" className="rounded-full" onClick={goPrev} aria-label="Zurück">
              <ArrowLeft className="size-4"/>
            </Button>
            <Button variant="ghost" size="icon-sm" className="rounded-full" onClick={goNext} aria-label="Weiter">
        ...
        <div className="flex items-center gap-0.5 rounded-full border border-border p-0.5">
          <Button variant={view === "month" ? "default" : "ghost"} size="sm" className="rounded-full" onClick={() => setView("month")}>
            Monat
          </Button>
          <Button variant={view === "week" ? "default" : "ghost"} size="sm" className="rounded-full" onClick={() => setView("week")}>
            Woche
          </Button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        <button type="button" onClick={() => setActiveCategory("All")} className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${activeCategory === "All"
            ? "bg-primary text-primary-foreground"
            : "bg-muted text-muted-foreground hover:text-foreground"}`}>
          {ALL_CATEGORIES_LABEL}
        </button>
        {categories.map((cat) => (<button key={cat} type="button" onClick={() => setActiveCategory(cat)} className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-colors ${activeCategory === cat
                ? "bg-primary text-primary-foreground"
                : "bg-muted text-muted-foreground hover:text-foreground"}`}>
      ...
      {view === "month" ? (<CalendarMonthView weeks={monthWeeks} tasks={visibleTasks} onSelectTask={setSelectedTask} onSelectDay={handleSelectDay}/>) : (<CalendarWeekView days={weekDays} tasks={visibleTasks} onSelectTask={setSelectedTask} onSelectSlot={handleSelectSlot}/>)}

      <DetailDialog open={selectedTask !== null} onClose={() => setSelectedTask(null)} title={selectedTask?.name}>
        {selectedTask && (<div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 text-sm">
              {selectedTask.description && (<div className="flex flex-col gap-0.5">
              ...
              {selectedTask.startTime && (<div className="flex gap-6">
                  ...
                  {selectedTask.durationMinutes && (<div className="flex flex-col gap-0.5">
                      <span className="text-xs text-muted-foreground">Dauer</span>
                      <span>{selectedTask.durationMinutes} Min.</span>
                    </div>)}
                </div>)}
              {selectedTask.dateUntil && (<div className="flex flex-col gap-0.5">
