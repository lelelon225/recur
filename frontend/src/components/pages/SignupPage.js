...
export default function SignupPage() {
    ...
    return (<div className="flex min-h-svh items-center justify-center bg-background w-full h-full px-4 py-8">
        ...
        <CardContent className="p-6">
            <Formik initialValues={{ firstName: "", lastName: "", email: "", password: "", confirmPassword: "" }} validationSchema={signupSchema} onSubmit={handleSubmit}>
                {({ values, handleChange, handleSubmit: formikHandleSubmit, handleBlur, errors, touched }) => (<SignupForm navigate={() => navigate("/login")} onSubmit={formikHandleSubmit} values={values} errors={errors} touched={touched} handleChange={handleChange} handleBlur={handleBlur} loading={loading} submitDisabled={submitDisabled} backendError={backendError}/>)}
            </Formik>
        </CardContent>
      </Card>
    </div>);
