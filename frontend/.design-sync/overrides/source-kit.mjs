// forked from design-sync lib/source-kit.mjs - the repo's components mostly
// use `export default`, and a synth-entry built from plain `export * from`
// silently drops every default export (ES module semantics: `export *`
// never forwards a module's default binding). This fork additionally emits
// an explicit `export { default as Name } from "path"` line per file that
// has a default export, so those components land on window.<Global> too.
...
// Returns [{name, isDefault}] per file so the synth entry (below) can emit
// the right re-export form for each.
function scanExports(project, p) {
...
function deriveComponentsFromSrc(srcFiles) {
  ...
  for (const p of srcFiles) {
    ...
    for (const { name } of scanExports(project, p)) seen.add(name);
  }
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // Two default exports can legitimately share a name (a locally-named
    // helper component that happens to collide with an unrelated
    // component's real name) - `export { default as Name } from ...` can
    // only bind one winner. cfg.componentSrcMap pins which file wins;
    // componentSrcMap: {Name: null} drops the name from the entry entirely;
    // otherwise first-seen wins and every other file with that default name
    // is dropped with a console warning (still reachable by pinning).
    const defaultOwner = new Map(); // name -> path
    for (const p of comps) {
      for (const { name, isDefault } of scanExports(project, p)) {
        if (!isDefault) continue;
        const pin = srcMap[name];
        if (pin === null) continue; // excluded
        if (typeof pin === 'string') {
          if (slash(resolve(PKG_DIR, pin)) !== p) continue; // pinned to a different file
          defaultOwner.set(name, p);
          continue;
        }
        if (defaultOwner.has(name)) {
          if (defaultOwner.get(name) !== p) {
            console.error(
              `[CONFIG] duplicate default export "${name}" in ${relative(PKG_DIR, p)} — keeping ${relative(PKG_DIR, defaultOwner.get(name))}. ` +
                `Pin the intended file with componentSrcMap.${name}, or componentSrcMap.${name}: null to drop both.`,
            );
          }
          continue;
        }
    ...
    for (const [name, p] of defaultOwner) lines.push(`export { default as ${name} } from ${JSON.stringify(p)};`);
    ...
    console.error(
      `[NO_DIST] no built entry — synthesizing from ${comps.length} src files ` +
        `(${defaultOwner.size} default-export re-exports; run the package's build for best results) [OVERRIDE: source-kit.mjs]`,
    );
  }
  ...
  // No dist (synthEntry) -> base list comes from the same src/ scan the
  // entry was synthesized from (upstream bug: gating this scan behind
  // `!names.length` means ANY componentSrcMap pin short-circuits the whole
  // 198-component auto-discovery down to just the pinned name(s) - fixed
  // here by always deriving from src/ first when synthEntry, same as
  // upstream's own fallback intent).
  // Override: cfg.componentSrcMap (non-null adds/pins, null excludes).
  const exported = exportedNames(PKG_DIR, pkgJson);
  const names = synthEntry
    ? new Set(deriveComponentsFromSrc(srcFiles).map((c) => c.name))
    : new Set([...exported].filter(isComponentName));
